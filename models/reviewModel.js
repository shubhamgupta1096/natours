...
const reviewSchema = new mongoose.Schema(
  {
    ...
    rating: {
      ...
      min: 1,
      max: 5,
      required: [true, 'Please provide some rating'],
    },
    createdAt: {
      type: Date,
      default: Date.now(),
...
reviewSchema.pre(/^findOneAnd/, async function (next) {
  ...
  //Since we can chain queries and the this keyword points to the current query and simply adding the findOne query gives us the current document in question
  next();
});
...
//Better way for the above two commands
